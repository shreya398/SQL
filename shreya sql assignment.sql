...
('Afghanistan',1593989,'Kabul'),
('India',1393892,'Delhi'),
('United States',324,'Washington D.C.'),
('Albania',6000,'Tirane'),
('Australia',7000,'Canberra'),
('Pakistan',1000,'Islamabad'),
('Bangladesh ',15000,'dhaka'),
(' Belarus',13000 ,'Minsk'),
('Bhutan ',15003 ,'Thimphu'),
('Mexico',1800,'Mexico City'),
('Japan',12900,'Tokyo'),
('Philippines', 1002 ,'Manila'),
('Brazil ',1008,'Brasilia'),
('Vietnam',3007,'Hanoi'),
('Bulgaria ',93,'Sofia'),
('Germany' ,81,'Berlin'),
('Chile',80,'Santiago'),
...
select* from countries;
insert into countries(name,capitals) values
('Jamaica', 'Kingston'),
('Japan', 'Tokyo'),
('Libya ', 'Tripoli'),
('Malawi','Lilongwe');
...
select*from countries;
update countries set capitals = "new delhi" where name ="india";
...
-- 7. Modify the supplier table to make supplier name unique and not null.
    ...
    alter table supplier modify supplier_name varchar(30) unique not null;
...
create unique index emp_no on emp(emp_no);
select*from emp;
...
select emp_no, first_name,last_name from emp  order by  salary desc;
select* from emp_sal;
...
select count(marks) students where marks >60 ;

-- b. How many students have obtained distinction? [table: students]
select count(marks) students where marks >80;
...
-- [table: station]
 select count(city) -count( distintcity) from station;
...
SELECT first_name ,hire_date,salary FROM EMP WHERE salary >2000 
and hire_date < hire_sub(current data, intervel36 months) order by salary desc;
...
select average(population) ,countrycode from city where COUNTRYCODE IN('JPN','USA','NLD')group by countrycode;


-- 11. Find the ordernumber, status, customernumber, customername and comments for all orders 
-- that are ‘Disputed=  [table: orders, customers]
...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `ORDER_STATUS`(MONTH1 varchar(3),YEAR1 INTEGER)
BEGIN
SELECT ordernumber, orderdate ,status FROM ORDERS
WHERE left(monthname(ORDERDATE),3)=MONTH1 AND YEAR(ORDERDATE)= YEAR1;
END$$
DELIMITER ;



-- 2A) Write a stored procedure to insert a record into the cancellations table for all cancelled orders.
